...
BEGIN
  ...
  FROM (
    SELECT pais_id FROM f_scl_pais_1@sclbdd_s1.fi.unam
    UNION ALL
    SELECT pais_id FROM f_scl_pais_2
  );

  --Conteo de oficinas
  SELECT COUNT(*) AS INTO v_num_oficinas
  FROM (
    SELECT oficina_id FROM f_scl_oficina_1@sclbdd_s1.fi.unam
    UNION ALL
    SELECT oficina_id FROM f_scl_oficina_2
  );

  --Conteo de proyectos
  SELECT COUNT(*) AS INTO v_num_proyectos
  FROM (
    SELECT proyecto_id FROM f_scl_proyecto_1@sclbdd_s1.fi.unam
    UNION ALL
    SELECT proyecto_id FROM f_scl_proyecto_2
    UNION ALL
    SELECT proyecto_id FROM f_scl_proyecto_3
  );
  ...
  FROM (
    ...
    FROM f_scl_proyecto_pdf_1@sclbdd_s1.fi.unam
    UNION ALL
    SELECT num_pdf, proyecto_id
  ...
  FROM (
    SELECT E1.empleado_id FROM f_scl_empleado_1 E1
    JOIN (
      SELECT empleado_id FROM f_scl_empleado_2@sclbdd_s1.fi.unam
      UNION ALL
      SELECT empleado_id FROM f_scl_empleado_3
  ...
  FROM (
    SELECT pago_empleado_id FROM f_scl_pago_empleado_1@sclbdd_s1.fi.unam
    UNION ALL
    SELECT pago_empleado_id FROM f_scl_pago_empleado_2
  );
